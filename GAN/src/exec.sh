...
python3 gan_ensemble.py --networks=$N_NETS --traj=$N_TRAJ --gen_lr=$GEN_LR --discr_lr=$DISCR_LR --timesteps=$TIMESTEPS --noise_timesteps=$NOISE_TIMESTEPS --model=$MODEL --epochs=$EPOCHS --gen_epochs=$GEN_EPOCHS --fixed_params=$FIXED_PARAMS --batch_size=$BATCH_SIZE &>> $OUT
python3 ensemble_evaluation.py --networks=$N_NETS --traj=5 --gen_lr=$GEN_LR --discr_lr=$DISCR_LR --timesteps=$TIMESTEPS --noise_timesteps=$NOISE_TIMESTEPS --model=$MODEL --epochs=$EPOCHS --gen_epochs=$GEN_EPOCHS --fixed_params=$FIXED_PARAMS --batch_size=$BATCH_SIZE &>> $OUT
