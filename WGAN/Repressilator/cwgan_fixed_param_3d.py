...
PLOTS_PATH = "Plots/3d_Emb_"+embedding+"_Plots_EP={}_BAT={}_LRs={}_{}_N_CR={}_N_CH={}_Q={}_CC={}".format(N_EPOCHS,BATCH_SIZE,C_LR,G_LR, N_CRITIC, N_CH, Q, CLIP_CONST)
try:
    os.mkdir(PLOTS_PATH)
except OSError:
...
MODELS_PATH = "Models/3d_SavedModels_EP={}_BAT={}_LRs={}_{}_N_CR={}_N_CH={}_Q={}_CC={}".format(N_EPOCHS,BATCH_SIZE,C_LR,G_LR, N_CRITIC, N_CH, Q, CLIP_CONST)
try:
...
def define_critic():
	...
	traj = Input(shape=(TRAJ_LEN+1, N_RED)) 
...
def define_generator(latent_dim):
	...
	# output
	outputs = Conv1D(N_RED, KG[-1], activation='tanh', padding='same')(x)
...
def define_gan(generator, critic):
	...
	in_st = Reshape((1,N_RED))(init_states)
...
def load_real_samples(filename):
	...
	HMAX = np.max(np.max(X, axis = 0),axis=0)/2
...
def generate_latent_points(latent_dim, initial_states, n_samples,  phase, ix = []):
	if phase == "D":
		...
		t_input = (rand(int(n_samples),N_RED)-0.5)*2	
	else:
		print("ERROR!!")
...
def generate_cond_fake_samples(generator, latent_dim, initial_state, n_samples):
	...
	# predict outputs
	initial_state_rep = initial_state*np.ones((n_samples,N_RED))
...
def summarize_performance(step, g_model, latent_dim, dataset, n_samples=4):
	...
	if intermediate_plots:
		for i in range(GR * GR):
			...
			pyplot.plot(xxx, complete_traj_fake[:, 0], label="S1", color = "r")
			pyplot.plot(xxx, complete_traj_fake[:, 1], label="S2", color = "b")
			pyplot.plot(xxx, complete_traj_fake[:, 2], label="S3", color = "g")

			pyplot.plot(xxx, complete_traj_real[:, 0], '--', label="S1", color = "r")
			pyplot.plot(xxx, complete_traj_real[:, 1], '--', label="S2", color = "b")
			pyplot.plot(xxx, complete_traj_real[:, 2], '--', label="S3", color = "g")
...
def train(g_model, c_model, gan_model, dataset, latent_dim, n_epochs=100, n_batch=64, n_critic=5, n_gen=1):
	...
	for i in range(n_steps):
		...
		for _ in range(n_critic):
			...
			t_real = tf.reshape(t_real, (half_batch,1, N_RED))
			...
			# update critic model weights
			t_fake = tf.reshape(t_fake, (half_batch,1, N_RED))
...
def load_test_data(HMAX, filename):
...
def compute_trajectories(generator, valid_data):
	...
	gen_trajectories = np.empty(shape=(len(initial_states), traj_per_state, TRAJ_LEN, N_RED))
...
def plot_trajectories(ssa_trajectories, gen_trajectories):#valid_dict
	...
	for init_state in range(n_init_states):
		...
		fig, ax = pyplot.subplots(N_RED,1,figsize=(12,6))
		...
		for s in range(n_species):
			...
			for traj_idx in range(5):#(traj_per_state):
				...
				#ax[s].legend()
		pyplot.tight_layout()
		fig.savefig(PLOTS_PATH+"/Repr_Trajectories"+str(init_state)+".png")
		pyplot.close()
...
def compute_distances(valid_data, gen_trajectories):#valid_dict,initial_states
	...
	bins = 100
	ssa_histograms_count = np.zeros(shape=(len(initial_states), TRAJ_LEN, N_RED, bins))
	ssa_histograms_edg = np.zeros(shape=(len(initial_states), TRAJ_LEN, N_RED, bins+1))
	gen_histograms_count = np.zeros(shape=(len(initial_states), TRAJ_LEN, N_RED, bins))
	gen_histograms_edg = np.zeros(shape=(len(initial_states), TRAJ_LEN, N_RED, bins+1))
	dist = np.zeros(shape=(len(initial_states), TRAJ_LEN, N_RED))
	...
	pyplot.tight_layout()
	figname = "Results/Transient_Traj_avg_wass_distance.png"
	fig.savefig(figname)
	...
	distances_dict = {"ssa_count":ssa_histograms_count, "ssa_edg":ssa_histograms_edg,
	              ...
	              "wass_dist":dist}
	file = open('Transient_FixedParam_wgan_conv1d_distances.pickle', 'wb')
	# dump information to that file
	...
	for s, init_state in enumerate(initial_states):
		...
		figname = PLOTS_PATH+"/hist_comparison_last_timestep_"+str(init_state)+".png"
		fig.savefig(figname)
		pyplot.close()
...
trainset_file = 'Datasets/Repressilator_training_set_fixed_param_transient_dt=1_red.pickle'
valset_file = 'Datasets/Repressilator_validation_set_fixed_param_transient_dt=1_red.pickle'
...
# load image data
dataset, HMAX = load_real_samples(trainset_file)
...
filename_gen = 'transient_traj_cwgan_fixed_param_generator_model_final.h5'
trained_generator.save(filename_gen)


valid_data = load_test_data(HMAX)
