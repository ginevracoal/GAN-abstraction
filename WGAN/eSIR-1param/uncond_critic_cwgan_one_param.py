...
PLOTS_PATH = "Plots/ONE_PARAM_emb_"+embedding+"_Plots_EP={}_BAT={}_LRs={}_{}_N_CR={}_N_CH={}_Q={}_CC={}".format(N_EPOCHS,BATCH_SIZE,C_LR,G_LR, N_CRITIC, N_CH, Q, CLIP_CONST)
try:
    os.mkdir(PLOTS_PATH)
except OSError:
...
MODELS_PATH = "Models/ONE_PARAM_SavedModels_EP={}_BAT={}_LRs={}_{}_N_CR={}_N_CH={}_Q={}_CC={}".format(N_EPOCHS,BATCH_SIZE,C_LR,G_LR, N_CRITIC, N_CH, Q, CLIP_CONST)
try:
...
def define_critic():
	...
	par = Input(shape=(PARAM_DIM,))

	if embedding== "DENSE":
		p = Dense(TRAJ_LEN+1)(par)
		p = Reshape((TRAJ_LEN+1,1))(p)
	else:
		p = RepeatVector(TRAJ_LEN+1)(par)
...
def define_generator(latent_dim):
	...
	if embedding== "DENSE":
		...
		p = RepeatVector(Q)(par)

	merge = Concatenate(axis=2)([iv, p, nv])
...
def define_gan(generator, critic):
	...
	gan_output = critic([merged_traj,par])
...
def load_real_samples():
	...
	# load dataset
	file = open('Datasets/eSIR_training_set_one_param.pickle', 'rb')
...
def generate_real_samples(dataset, n_samples):
	...
	trajectories, initial_states, par = dataset 
...
def generate_latent_points(latent_dim, initial_states, par, n_samples,  phase, ix = []):
	if phase == "D":
		t_input = initial_states[ix]
		p_input = par[ix]
	elif phase == "G":	
		t_input = (rand(int(n_samples),N_SPECIES)-0.5)*2
		p_input = (rand(int(n_samples),PARAM_DIM)-0.5)*2
	else:
		print("ERROR!!")
...
def generate_cond_fake_samples(generator, latent_dim, initial_state, selected_par, n_samples):
	# generate points in latent space
	z_input = generate_noise(latent_dim, n_samples)
	# predict outputs
	initial_state_rep = initial_state*np.ones((n_samples,N_SPECIES))
	
	par_rep = selected_par*np.ones((n_samples,PARAM_DIM))
	
	X_gen = generator.predict([z_input, initial_state_rep, par_rep])
	
	return X_gen 
 
# use the generator to generate n fake examples, with class labels
def generate_fake_samples(generator, initial_states, selected_par, latent_dim, n_samples, phase, ret_ind = False, ix = []):
	# generate points in latent space
	z_input, t_input, p_input, idx = generate_latent_points(latent_dim, initial_states, selected_par, n_samples, phase, ix = ix)
	# predict outputs
	X = generator.predict([z_input, t_input, p_input])
...
def train(g_model, c_model, gan_model, dataset, latent_dim, n_epochs=100, n_batch=64, n_critic=5, n_gen=1):
	trajectories, initial_states, parameters = dataset
	...
	for i in range(n_steps):
		...
		for _ in range(n_critic):
			...
			X_real, t_real, p_real, y_real, idx = generate_real_samples(dataset, half_batch)
			...
			c_loss1 = c_model.train_on_batch([real_traj, p_real], y_real)
			c1_tmp.append(c_loss1)
			# generate 'fake' examples
			X_fake, t_fake, p_fake, y_fake = generate_fake_samples(g_model, initial_states, parameters, latent_dim, half_batch, phase="D", ix = idx)
			# update critic model weights
			t_fake = tf.reshape(t_fake, (half_batch,1, N_SPECIES))
			fake_traj = tf.concat([t_fake, X_fake], axis = 1)
			c_loss2 = c_model.train_on_batch([fake_traj, p_fake], y_fake)
		...
		for _ in range(n_gen):
			...
			X_gan, t_gan, p_gan, _ = generate_latent_points(latent_dim, initial_states, parameters, n_batch, phase="G")
			# create inverted labels for the fake samples
			y_gan = -ones((n_batch, 1))
			# update the generator via the critic's error
			g_loss = gan_model.train_on_batch([X_gan, t_gan, p_gan], y_gan)
			g_tmp.append(g_loss)
...
def load_test_data():
	...
	file = open('Datasets/eSIR_validation_set_one_param.pickle', 'rb')
	# dump information to that file
...
def compute_trajectories(generator, valid_data):
	...
	ssa_trajectories, initial_states, par = valid_data
	...
	for s, init_state in enumerate(initial_states):
		...
		gen_traj = generate_cond_fake_samples(generator, noise_timesteps, init_state, par[s], traj_per_state)

		gen_trajectories[s, :, :, :] = gen_traj			
		
		
	valid_dict = {"ssa": ssa_trajectories, "gen": gen_trajectories}
	file = open('Results/OneParam_wgan_conv1d_results.pickle', 'wb')
	# dump information to that file
...
def compute_distances(valid_data, gen_trajectories):#valid_dict,initial_states
	...
	figname = "Results/ONE_PAR_Traj_avg_wass_distance_{}epochs.png".format(N_EPOCHS)
	fig.savefig(figname)
	distances_dict = {"ssa_count":ssa_histograms_count, "ssa_edg":ssa_histograms_edg,
	              ...
	              "wass_dist":dist}
	file = open('Results/ONE_PAR_FixedParam_wgan_conv1d_distances.pickle', 'wb')
	# dump information to that file
...
if DO_TRAIN:
	...
	filename_gen = 'Models/latest_cwgan_fixed_param_traj_generator_model_{}_epochs.h5'.format(N_EPOCHS)
...
ssa_valid_trajectories, valid_init_states, valid_params = valid_data
