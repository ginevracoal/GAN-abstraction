...
PLOT_PATH = "Plots/2D_enriched_ds={}-steps_FIXED_FF_CGAN_ngen_{}_nepochs_{}_ZZ ={}_LR={}".format(Nsteps, Ngen, N_EPOCHS, ZZ, LR)
try:
    os.mkdir(PLOT_PATH)
except OSError:
...
class CDCGAN_KERN(object):
	...
	def train(self, n_epochs=10, n_batch=64, n_gen=5):
		...
		filename2 = 'Models/trained_gen_model_final_ep_{}.h5'.format(N_EPOCHS)
	...
	def load_test_data(self, val_data_filename):
		...
		x1_val = data["X"]
		x0_val = data["Y_s0"]
		print("TEST_X1 SHAPE: ", x1_val.shape)
	...
	def plot_test_hist(self, gen_trajs):
		...
		for i, init_state in enumerate(self.x0_val):
			for t in range(self.T):
				...
				fig.savefig(figname)
				pyplot.close()
		fname = 'TransientResults/large_enriched_wass_dist_{}steps.npy'.format(Nsteps)
		...
		figname = "TransientResults/large_enriched_avg_wass_distance_{}steps.png".format(Nsteps)
		fig.savefig(figname)
...
cdcgan.define_gan()
# load image data
#cdcgan.load_real_samples('Datasets/TS_training_set_kernel_red_128steps.pickle')
if ENRICH_DS:
...
#cdcgan.load_test_data('Datasets/TS_validation_set_kernel_red_128steps.pickle')
