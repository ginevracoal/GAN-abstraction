...
PMAX = 2.5 #np.array([4.5/2])

PLOT_PATH = "Plots/{}-Step_FIXED_FF_CGAN_ngen_{}_nepochs_{}_ZZ ={}".format(Nsteps, Ngen, N_EPOCHS, ZZ)
try:
    os.mkdir(PLOT_PATH)
except OSError:
...
class CGAN_KERN_1P(object):

	def __init__(self, noise_dim, state_dim, param_dim):
	...
	def define_discriminator(self):
		...
		inputs = Concatenate(axis=1)([x1, x0, par])
		...
		model = Model(inputs=[x1, x0, par], outputs=output_l)
	...
	def define_generator(self):
		...
		merge = Concatenate(axis=1)([noise, x0, par])
		...
		model = Model(inputs=[noise, x0, par], outputs=output_x1)
	...
	def define_gan(self):
		...
		noise, x0, par = self.generator.input
		gen_x1 = self.generator.output

		gan_output = self.discriminator([gen_x1, x0, par])

		model = Model(inputs=[noise, x0, par], outputs=gan_output)
	...
	def generate_latent_points(self, n_samples, phase, ix = []):
		...
		if phase == "D":
			x0_input = self.x0[ix]
			par_input = self.par[ix]
		elif phase == "G":	
			if self.normalization:
				x0_input = (rand(int(n_samples),self.state_dim)-0.5)*2
				par_input = (rand(int(n_samples),self.param_dim)-0.5)*2
			else:
				x0_input = 2*HMAX*rand(int(n_samples),self.state_dim)
				par_input = 2*PMAX*rand(int(n_samples),self.param_dim)
		else:
			print("ERROR!!")
		...
		return z_input, x0_input, par_input, ix
	...
	def generate_cond_fake_samples(self, selected_x0, selected_par, n_samples):
		...
		gen_x1 = self.generator.predict([z_input, x0_rep, par_rep])
		
		return gen_x1 


	def generate_cond_fake_trajs(self, selected_x0, selected_par, n_samples):
		# generate points in latent space
		...
		for t in range(self.T):
			z_input = self.generate_noise(n_samples)
			# predict outputs		
			gen_x1 = self.generator.predict([z_input, init_states, par_rep])
			gen_trajs[:,t,:] = gen_x1
			init_states = gen_x1

		return gen_trajs 

	def generate_cond_fake_unscaled_trajs(self, selected_x0, selected_par, n_samples):
		# generate points in latent space
	...
	def generate_fake_samples(self, n_samples, phase, ret_ind = False, ix = []):
		# generate points in latent space
		z_input, x0_input, par_input, idx = self.generate_latent_points(n_samples, phase, ix = ix)
		# predict outputs
		gen_x1 = self.generator.predict([z_input, x0_input, par_input])
		# create class labels with 1.0 for 'fake'
		lab = zeros((n_samples, 1))
		return gen_x1, x0_input, par_input, lab
	...
	def summarize_performance(self, step, n_samples=4):
		...
		par_inputs = np.array([self.par[ID[i]*hist_size:(ID[i]+1)*hist_size] for i in range(n_samples)])
		
		gen_x1 = np.empty((n_samples, hist_size, self.state_dim))
		for j in range(n_samples):
			gen_x1[j] = self.generate_cond_fake_samples(x0_inputs[j], par_inputs[j], hist_size)
	...
	def train(self, n_epochs=10, n_batch=64, n_gen=5):
		...
		for i in range(n_steps):
			...
			x1_real, x0_real, par_real, l_real, idx = self.generate_real_samples(half_batch)
			#print(X_real.shape, t_real.shape, y_real.shape)
			# update critic model weights
			d_loss1, d_acc1 = self.discriminator.train_on_batch([x1_real, x0_real, par_real], l_real)
			# generate 'fake' examples
			x1_fake, x0_fake, par_fake, l_fake = self.generate_fake_samples(half_batch, phase="D", ix = idx)
			# update critic model weights
			
			d_loss2, d_acc2 = self.discriminator.train_on_batch([x1_fake, x0_fake, par_fake], l_fake)
			...
			for _ in range(n_gen):
				...
				x1_gan, x0_gan, par_gan, _ = self.generate_latent_points(n_batch, phase="G")
				# create inverted labels for the fake samples
				l_gan = ones((n_batch, 1))
				# update the generator via the critic's error
				g_loss = self.gan.train_on_batch([x1_gan, x0_gan, par_gan], l_gan)
				g_tmp.append(g_loss)
		...
		filename2 = 'Models/trained_gen_model_final.h5'
	...
	def load_test_data(self, val_data_filename):
		...
		if self.normalization:
			...
			self.par_val = (par_val-PMAX)/PMAX
	...
	def compute_test_results(self, scaled_flag):
		...
		for i, init_state in enumerate(self.x0_val):
			...
			if scaled_flag:
				gen_traj = self.generate_cond_fake_trajs(init_state, self.par_val[i], self.traj_per_state)
			else:
				gen_traj = self.generate_cond_fake_unscaled_trajs(init_state,  self.par_val[i], self.traj_per_state)
	...
	def plot_test_hist(self, gen_trajs, scaled_flag):
		...
		for i, init_state in enumerate(self.x0_val):
			for t in range(self.T):
				...
				fig.savefig(figname)
				pyplot.close()
		fname = 'Results/Scaled={}_wass_dist_{}steps.npy'.format(scaled_flag, Nsteps)
		...
		figname = "Results/Scaled={}_avg_wass_distance_{}steps.png".format(scaled_flag, Nsteps)
		fig.savefig(figname)
...
cgan = CGAN_KERN_1P(latent_dim, state_dim, param_dim)
cgan.set_T(Nsteps)
cgan.load_real_samples('Datasets/eSIR_training_set_one_param.pickle')
cgan.load_test_data('Datasets/eSIR_validation_set_one_param.pickle')
...
if DO_TRAIN:
	# train model
	cgan.train(n_epochs = N_EPOCHS, n_batch = 256, n_gen=Ngen)
else:
	cgan.generator = load_model('Models/best_model_final.h5')
