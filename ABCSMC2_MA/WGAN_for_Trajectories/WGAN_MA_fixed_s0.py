...
class WGAN_MA_fixed_s0(object):

	def __init__(self, model_name, noise_dim, state_dim, param_dim, traj_len, labels, colors, embedding):
	...
	def generate_directories(self, ID = None):
		if not ID:
	...
	def define_critic(self):
		...
		traj = Input(shape=(self.traj_len, self.state_dim)) 
		par = Input(shape=(self.param_dim,))

		if self.embedding == "DENSE":
			p = Dense(self.traj_len)(par)
			p = Reshape((self.traj_len,1))(p)
		else:
			p = RepeatVector(self.traj_len)(par)
	...
	def define_generator(self):
		...
		merge = Concatenate(axis=2)([pv,nv])
		...
		model = Model(inputs=[noise,par], outputs=outputs)
	...
	def define_gan(self):
		...
		noise, par = self.generator.input
		gen_traj = self.generator.output

		
		gan_output = self.critic([gen_traj, par])

		model = Model(inputs=[noise, par], outputs=gan_output)
	...
	def generate_latent_points(self, n_samples,  phase, ix = []):
		if phase == "D":
			...
			t_input = (rand(int(n_samples),self.param_dim)-0.5)*2	
		else:
			print("ERROR!!")
	...
	def generate_cond_fake_samples(self, param, n_samples):
		# generate points in latent space
		z_input = self.generate_noise(n_samples)
		# predict outputs
		param_rep = param*np.ones((n_samples,self.param_dim))
		
		X_gen = self.generator.predict([z_input, param_rep])
	...
	def summarize_performance(self, step, n_samples=4):
		...
		if self.intermediate_plots_flag:
			for i in range(GR * GR):
				...
				xxx = np.linspace(1,self.traj_len,self.traj_len)
				for spec in range(self.state_dim):
					pyplot.plot(xxx, X_fake[i, :, spec], label=self.labels[spec], color = self.colors[spec])
					pyplot.plot(xxx, X_real[i, :, spec], '--', label=self.labels[spec], color = self.colors[spec])
	...
	def train(self):
		...
		for i in range(n_steps):
			...
			for _ in range(self.n_critic):
				...
				c_loss1 = self.critic.train_on_batch([X_real, t_real], y_real)
				c1_tmp.append(c_loss1)
				# generate 'fake' examples
				X_fake, t_fake, y_fake = self.generate_fake_samples(half_batch, phase="D", ix = idx)
				# update critic model weights
				c_loss2 = self.critic.train_on_batch([X_fake, t_fake], y_fake)
			...
			for _ in range(self.n_gen):
				...
				Z_gan, t_gan, _ = self.generate_latent_points(self.batch_size, phase="G")
				# create inverted labels for the fake samples
				y_gan = -ones((self.batch_size, 1))
				# update the generator via the critic's error
				g_loss = self.gan.train_on_batch([Z_gan, t_gan], y_gan)
				g_tmp.append(g_loss)
	...
	def generate_validation_trajectories(self):
		...
		print(f"\nComputing trajectories on {len(self.T_val)} params")
		gen_trajectories = np.empty(shape=(len(self.T_val), traj_per_state, self.traj_len, self.state_dim))

		for pi, params in enumerate(self.T_val):
			print("\tparams = ", params)
			
			gen_traj = self.generate_cond_fake_samples(params, traj_per_state)

			gen_trajectories[pi, :, :, :] = gen_traj			
	...
	def eval_times(self):
		...
		for pi, param in enumerate(self.T_val):
			print("\tparams = ", params)
			start_time = time.time()
			gen_traj = self.generate_cond_fake_samples(param, traj_per_state)
	...
	def plot_validation_trajectories(self):
		...
		n_params, traj_per_state, n_timesteps, n_species = self.X_val.shape
		...
		for pi in range(n_params):
			...
			ssa_fixed_init = ssa_trajectories_unscaled[pi]
			gen_fixed_init = gen_trajectories_unscaled[pi]
			...
			fig.savefig(self.PLOTS_PATH+"/"+self.MODEL_NAME+"_Trajectories"+str(pi)+".png")
			pyplot.close()


	def plot_t_step_histogram(self, t):
		...
		for pi, param in enumerate(self.T_val):
			fig, ax = pyplot.subplots(self.state_dim,1, figsize = (12,self.state_dim*3))
			for spec in range(self.state_dim):
				XXX = np.vstack((ssa_trajectories_unscaled[pi,:,t,spec], gen_trajectories_unscaled[pi,:,t,spec])).T
				
				ax[spec].hist(XXX, bins = bins, stacked=False, density=False, color=colors, label=leg)
				ax[spec].legend()
				ax[spec].set_ylabel(self.labels[spec])

			figname = self.PLOTS_PATH+"/"+self.MODEL_NAME+"_hist_comparison_"+str(t)+"th_timestep_"+str(pi)+".png"
			fig.savefig(figname)
	...
	def compute_distances(self):
		...
		print(f"\nComputing histograms on {len(self.T_val)} parameters")
		...
		for pi, param in enumerate(self.T_val):
			...
			for t in range(self.traj_len):
				for m in range(self.state_dim):
					A = ssa_trajectories_unscaled[pi,:,t,m]
					B = gen_trajectories_unscaled[pi,:,t,m]

					histA, edgA = np.histogram(A, bins=np.arange(XMAX))

					histB, edGB = np.histogram(B, bins=np.arange(XMAX))
					
					dist[pi, t, m] = wasserstein_distance(histA, histB)
